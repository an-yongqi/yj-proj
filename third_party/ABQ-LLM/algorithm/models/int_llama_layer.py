...
from quantize.abq_norm import AbqLlamaRMSNorm
...
class QuantLlamaMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.down_proj(hidden_states), hidden_states
...
class QuantLlamaDecoderLayer(nn.Module):
    def __init__(self, 
        ...
        self.input_layernorm = AbqLlamaRMSNorm(ori_layer.input_layernorm,eps=ori_layer.input_layernorm.variance_epsilon)
        self.post_attention_layernorm = AbqLlamaRMSNorm(ori_layer.post_attention_layernorm,eps=ori_layer.post_attention_layernorm.variance_epsilon)
        # self.attn = None
    def forward(
        self,
        hidden_states: torch.Tensor,
        ...
        **kwargs,  # absorb cache_position etc. from transformers>=4.42
        ...
        hidden_states, down = self.mlp(hidden_states)
        hidden_states = residual + hidden_states

        outputs = (hidden_states,)

        if output_attentions:
            self_attn_weights = torch.mean(self_attn_weights, dim=1)
    ...
    def abq_parameters(self, use_shift=True):
        params = []
        template = "smooth" if use_shift else "smooth_scale"
        for n, m in self.named_parameters():
    ...
    def abq_state_dict(self, destination=None, prefix='', keep_vars=False):
